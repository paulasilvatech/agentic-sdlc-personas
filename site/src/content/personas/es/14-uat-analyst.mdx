...
> The UAT Analyst es la persona que represents the business inside the Verification phase. En un SDLC AI-native, el UAT Analyst operates an Acceptance Scribe agent that converts business intent into executable acceptance evidence — not a static sign-off form.

## Resumen ejecutivo

The UAT Analyst closes the loop between the Business Analyst's requirements and the end user's reality. Where the QA Engineer proves the system is built right, the UAT Analyst proves it is the right system. En un SDLC AI-native, that proof is assembled with a single Acceptance Scribe agent, cuatro slash prompts, instrucciones con alcance, and a validated MCP catalog that reaches into Azure DevOps Test Plans, GitHub, and Playwright for recorded walkthroughs.
...
Piense en el UAT Analyst like a theatre dramaturg running final dress rehearsal. The director, writer, and actors all have their own view; the dramaturg ensures the audience will understand what is happening and that the piece honors the source material. En un SDLC AI-native, el UAT Analyst is the persona accountable for the audience perspective on every feature before it goes live.
...
1. Como UAT Analyst, quiero every business requirement to have at least one executable acceptance scenario, para que sign-off is backed by evidence, not opinion.
2. Como UAT Analyst, quiero UAT sessions scheduled automatically from the release plan, para que no feature ships without a dated rehearsal.
3. Como UAT Analyst, quiero recorded walkthroughs attached to each sign-off, para que audits and future regressions have an artifact to replay.
4. Como UAT Analyst, quiero divergences between signed scenarios and production behavior detected within one day, para que trust with the business is preserved.
5. Como UAT Analyst, quiero a single, queryable business-traceability matrix, para que leadership can answer "is requirement X verified in production?" in seconds.
6. Como UAT Analyst, quiero every scenario written in the business's own words, para que domain experts can approve without translation.
7. Como UAT Analyst, quiero sign-offs time-stamped and signed, para que governance and compliance audits are satisfied without manual packaging.
8. Como UAT Analyst, quiero the UAT plan distributed in Microsoft Teams and Loop, para que stakeholders see dates, owners, and outcomes in one place.
...
- **L2 Asistido**: Copilot drafts scenarios but humans manually copy them into a wiki; sign-offs are screenshots.
- **L3 Aumentado**: Acceptance Scribe agent, cuatro slash prompts, instrucciones con alcance, Playwright MCP recordings linked from sign-offs.
- **L4 Autónomo**: Nightly divergence detection, automated scheduling in Microsoft Teams, business-traceability matrix published daily to Loop, pre-release hook blocks unacceptable requirements automatically.

## Integración con otras personas

- **Del Business Analyst**: approved requirements with EARS statements and domain glossary updates.
- **Del Product Owner**: prioritized release plan indicating which requirements need UAT in this cycle.
- **Para el Developer**: divergence issues with linked signed scenarios and Application Insights telemetry.
- **With QA Engineer**: shared Playwright fixtures; acceptance scenarios feed regression suite candidates.
- **Para el Compliance Auditor**: the business-traceability matrix and signed artifacts as audit evidence.
- **Para el Release Manager**: pre-release hook blocks on missing sign-offs.
