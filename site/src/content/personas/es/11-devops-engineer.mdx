...
> The DevOps Engineer es la persona que turns application code into deployable, observable infrastructure. En un SDLC AI-native, el DevOps Engineer opera un stack de primitivas validadas, not a stack of shell scripts.

## Resumen ejecutivo

The DevOps Engineer owns the path from commit to production: continuous integration, continuous delivery, infrastructure as code, environment promotion, and release trains. En un SDLC AI-native, el DevOps Engineer opera dentro de la fase de Operation phase con un conjunto fijo de primitivas: un agente pipeline, cuatro slash prompts, instrucciones con alcance, hooks validados por esquema, and una lista curada de MCPs validados. The primary outputs are GitHub Actions workflows, Bicep modules, Azure DevOps pipeline YAML, environment configurations, and release artifacts traceable back to the originating specification.

## Rol y responsabilidades

Piense en el DevOps Engineer like a track superintendent on a rail network. Trains (releases) must run on time, on the right track (environment), with the right cargo (artifact), and the switches (gates) must open only when the signals are green. The superintendent does not drive every train, but every train that moves safely does so because the track, signals, and switches were designed, tested, and maintained upstream. En un SDLC AI-native, el superintendent is a human operating a fleet of automated switches built from Bicep, GitHub Actions, and Azure DevOps pipelines.
...
1. Como DevOps Engineer, quiero scaffold a new service pipeline in minutes, para que new repos inherit the golden path without copy-paste.
2. Como DevOps Engineer, quiero Bicep changes reviewed by an agent that understands the resource graph, para que drift is caught before merge.
3. Como DevOps Engineer, quiero environment promotion to be a one-click operation with a machine-readable checklist, para que promotions are reproducible.
4. Como DevOps Engineer, quiero release train composition to be generated from merged PRs, para que cut day is a signoff, not a scramble.
5. Como DevOps Engineer, quiero every pipeline to emit OpenTelemetry traces into Azure Monitor, para que failed builds are debugged with data, not intuition.
6. Como DevOps Engineer, quiero secrets rotated automatically with Azure Key Vault references, para que no long-lived credential lives in a runner.
...
The DevOps Engineer operates a fixed loop each day. El loop usa primitivas de GitHub Copilot dentro de Visual Studio Code y Claude Code en la terminal, además de un pequeño catálogo de MCPs validados para contexto externo.

### Setup de la mañana

1. Open the platform repo in Visual Studio Code. GitHub Copilot Chat carga `AGENTS.md` and the scoped `.github/instructions/*.instructions.md` for Bicep, YAML, and PowerShell.
...
**Invocación**: `/pipeline-scaffold` with service name and tier.
...
1. **Inline YAML copy-paste**. Each service repo holding its own workflow diverges within a quarter. Mitigación: reusable workflows referenced by tag, owned by the platform team.
2. **Secrets in variables**. Long-lived `AZURE_CREDENTIALS` in GitHub Actions or Azure DevOps variable groups. Mitigación: OIDC federation with Entra ID; Key Vault references for runtime secrets.
3. **Bicep without parameter files**. Changing a parameter inline to deploy to another environment. Mitigación: un agente parameter file per environment, checked in, reviewed by the.
4. **Manual release notes**. Scraping commits the night before cut day. Mitigación: `/release-train` composes from merged PRs with risk tiers.
5. **Skipping what-if**. Deploying Bicep without a what-if preview. Mitigación: `pre-push` hook fails the push if what-if was not run.
...
| L2 | Asistido | GitHub Copilot autocomplete on YAML, Bicep adopted partially, OIDC for some environments |
| L3 | Aumentado | One Pipeline Smith agent, cuatro slash prompts, instrucciones con alcance, Azure MCP for what-if, reusable workflows |
...
- **Del Software Architect**: target topology, non-functional requirements, `IMPLEMENTATION_PLAN.md`
- **Del Developer**: merged PR with passing tests, deployment artifact
- **Para el Release Manager**: release train draft, risk tiers, rollback plan
- **Para el SRE**: deployed artifact, dashboards, SLO configuration
- **Para el InfoSec Officer**: policy compliance report, secret rotation audit, OIDC federation map
