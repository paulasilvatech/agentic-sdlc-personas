...
> The ML AI Engineer es la persona que brings models and LLM-powered features from prototype to production. En un SDLC AI-native, el ML AI Engineer operates an Eval Runner agent, cuatro slash prompts, and a validated MCP catalog centered on Azure AI Foundry, Azure OpenAI, and Azure Machine Learning — not a pile of ad-hoc notebooks.

## Resumen ejecutivo

The ML AI Engineer is responsible for the reliability of AI behavior: model quality, prompt stability, routing decisions, and deployment topology. En un SDLC AI-native, el role is operationalized through a single Eval Runner agent, cuatro slash prompts, instrucciones con alcance, and validated MCPs reaching into Azure AI Foundry, Azure OpenAI, Azure Machine Learning, and Microsoft Learn Docs MCP.
...
Piense en el ML AI Engineer like a pharmaceutical process engineer. The chemist invents the compound; the process engineer ensures every batch meets the monograph, with tests at each stage. En un SDLC AI-native, el ML AI Engineer turns exploratory model work into repeatable, auditable production behavior.
...
4. Keep prompts in `prompts/*.prompt.md` with instrucciones con alcance; no prompts inline in application code.
...
- **L2 Asistido**: Copilot drafts prompts and training code, evals in a notebook, single-model setup.
- **L3 Aumentado**: Eval Runner agent, cuatro slash prompts, instrucciones con alcance, Foundry deploys from GitHub Actions.
- **L4 Autónomo**: Prompt and routing changes gated by evals, telemetry feeding the next eval automatically, rollback on anomaly within five minutes.

## Integración con otras personas

- **Del Data Engineer**: curated feature datasets in Microsoft Fabric with documented lineage.
- **Del Software Architect**: AI topology decisions (which tier calls which model) and latency budgets.
- **Para el Developer**: prompt files and routing config consumed from the repo; no inline prompts.
...
- **Para el Product Owner**: eval dashboards and regression reports during release review.
