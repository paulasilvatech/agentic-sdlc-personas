...
> The Release Manager es la persona que decides what ships, when it ships, and how to undo it when it must. En un SDLC AI-native, el Release Manager opera un stack de primitivas validadas, not a last-minute spreadsheet.

## Resumen ejecutivo

The Release Manager owns the release lifecycle: composition, risk assessment, gating, communication, canary analysis, and rollback. En un SDLC AI-native, el Release Manager opera dentro de la fase de Release phase con un conjunto fijo de primitivas: un agente release, cuatro slash prompts, instrucciones con alcance, hooks validados por esquema, and una lista curada de MCPs validados. Releases are orchestrated through GitHub Releases and Azure DevOps release gates, communicated via Microsoft 365 and Teams, and monitored through Azure Monitor and Application Insights. The primary outputs are release notes, CHANGELOG entries, gated approvals, canary reports, and rollback plans.

## Rol y responsabilidades

Piense en el Release Manager like a harbor pilot. The ship was built by the shipyard, loaded by the stevedores, and captained by someone else, but it cannot enter the harbor without a pilot who knows the channel, the tides, and the traffic. The pilot's job is not to build, load, or captain; it is to ensure safe passage through the last mile. En un SDLC AI-native, el harbor is production, the tides are business hours and compliance windows, and the channel is the sequence of gates from build artifact to user impact.
...
1. Como Release Manager, quiero release notes generated from merged PRs, para que cut day is a signoff, not a scramble.
2. Como Release Manager, quiero every release to carry a risk tier and a rollback plan, para que go or no-go is a minutes-long meeting.
3. Como Release Manager, quiero canary signals read by an agent, para que human attention is spent on ambiguous cases only.
4. Como Release Manager, quiero CHANGELOG entries versioned alongside the code, para que downstream consumers never diff by hand.
5. Como Release Manager, quiero release communication sent to Teams and SharePoint automatically, para que the release channel is never the bottleneck.
6. Como Release Manager, quiero postmortems to feed forward into future release gates, para que the same incident never ships again.
...
The Release Manager operates a fixed loop each release cycle. El loop usa primitivas de GitHub Copilot dentro de Visual Studio Code y Claude Code en la terminal, además de un pequeño catálogo de MCPs validados para contexto externo.

### Setup de la mañana

1. Open the release coordination repo in Visual Studio Code. GitHub Copilot Chat carga `AGENTS.md` and the scoped `.github/instructions/release.instructions.md`.
...
**Invocación**: `/release-notes` followed by `/risk-gate`.
...
1. **Cut-day release notes**. Notes written the night before with scraped commits. Mitigación: `/release-notes` composes continuously from merged PRs, not at cut.
2. **Risk tier by feel**. Reviewers approve without a reproducible heuristic. Mitigación: `risk-heuristic` skill produces tier from PR size, blast radius, and security alert deltas.
3. **Rollback plan as "redeploy previous tag"**. No flag inventory, no data reversal. Mitigación: `/rollback-plan` produces the complete sequence; rehearsed in staging before release.
4. **Canary read by staring**. A human watches dashboards for 45 minutes. Mitigación: `/canary-report` reads Application Insights programmatically.
5. **Release communication by email forward**. Stakeholders learn about the release when alerts fire. Mitigación: Microsoft 365 Agents SDK MCP posts the train departure and arrival automatically.
...
| L2 | Asistido | GitHub Copilot helps draft notes, CHANGELOG maintained, no agent |
| L3 | Aumentado | One Release Scribe agent, cuatro slash prompts, instrucciones con alcance, two MCPs, canary read semi-automated |
...
- **Del DevOps Engineer**: release train candidate, risk tiers, what-if diffs
- **Del QA Engineer**: test matrix, regression report, known issues
- **Para el SRE**: deployed build, canary window, alert posture
- **Para el Tech Writer**: release notes, CHANGELOG, migration guides
- **Para el Product Owner**: stakeholder-facing release summary posted to SharePoint
