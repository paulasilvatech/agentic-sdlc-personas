...
> The UX Designer es la persona que curates the design system and guards accessibility. En un SDLC AI-native, el UX Designer opera un stack de primitivas validadas that turn design decisions into machine-readable tokens and enforceable gates.

## Resumen ejecutivo

The UX Designer produces and governs the design system, the information architecture, and the accessibility guarantees for every user-facing surface. En un SDLC AI-native, el UX Designer opera dentro de la fase de Design phase con un conjunto fijo de primitivas: un agente design system curation, cuatro slash prompts, instrucciones con alcance, hooks validados por esquema, and una lista curada de MCPs validados. Las salidas principales son the token map, the component pattern library, accessibility scan reports, and the information architecture that the Developer and QA Engineer implement against.

## Rol y responsabilidades

Piense en el UX Designer like the editor of a design language. They do not ship every screen, but every screen that ships has to match the typography, spacing, color, motion, and accessibility rules they authored. En un SDLC AI-native, el design language lives as machine-readable tokens, the pattern library is consumable by agents, and the UX Designer is accountable for the fact that the product looks and feels coherent from the first sketch to the rendered page.
...
1. Como UX Designer, quiero the token map to be the source of truth, para que every component and page reads from the same palette and scale.
2. Como UX Designer, quiero accessibility to be gated on PR, para que WCAG violations never reach production.
3. Como UX Designer, quiero the pattern library to be consumable by agents, para que Copilot proposes components from our system, not from the open web.
4. Como UX Designer, quiero the information architecture to bind journeys to pages, para que every screen has a named context and purpose.
5. Como UX Designer, quiero design tokens to regenerate the CSS and the Static Web App theme on merge, para que intent and output stay aligned.
6. Como UX Designer, quiero the design review digest to surface violations and patterns weekly, para que debt is managed, not accumulated.
...
The UX Designer operates a fixed loop each day. El loop usa primitivas de GitHub Copilot dentro de Visual Studio Code y Claude Code en la terminal, además de un pequeño catálogo de MCPs validados para contexto externo.

### Setup de la mañana

1. Open the design system repository in Visual Studio Code. GitHub Copilot Chat carga `AGENTS.md` and the scoped design system instructions.
2. Hacer pull de la última `main` y revisar cambios nocturnos component PRs against the pattern library.
...
2. Abrir un pull request on token, pattern, or IA changes. GitHub Copilot Code Review comments on token consistency and accessibility annotations.
...
Los hooks cuestan cero tokens de LLM. Son la capa de gobernanza más fuerte for design.
...
**Invocación**: `/token-map` followed by `/a11y-scan`.
...
**Invocación**: `/ia-review` followed by `/a11y-scan`.
...
1. **Copy-paste design tokens**. Values diverge across repos and platforms. Mitigación: token files are the single source, regenerated by hook.
2. **Accessibility as a release checklist**. Violations land in production before the checklist runs. Mitigación: `pre-merge` hook runs `/a11y-scan` on every PR preview.
3. **Pattern library as PDF**. Developers invent components off-system. Mitigación: the pattern library is markdown and the primed-context pack links patterns by name.
4. **Journeys drawn, not mapped**. Pages ship without named context, analytics, or accessibility annotations. Mitigación: `/ia-review` binds every page to pattern and analytics references.
5. **Design debt tolerated**. Divergences accumulate without a scan. Mitigación: weekly design digest surfaces divergence counts with owners.
...
| L2 | Asistido | Copilot used to polish pattern prose, tokens copy-pasted to CSS |
| L3 | Aumentado | Design System Curator agent, cuatro slash prompts, instrucciones con alcance, Playwright MCP, a11y scans on PR |
| L4 | Autónomo | Full primitives kit, hooks enforced, token regeneration automated, IA bindings standard, weekly digest live |

## Integración con otras personas

- **Del Product Owner**: approved `SPECIFICATION.md` with user-facing acceptance criteria that the IA must honor
- **Del Enterprise Architect**: principles that constrain visual and accessibility policy
- **Para el Developer**: tokens, patterns, and primed-context packs that make component work deterministic
- **Para el QA Engineer**: accessibility scans as a first-class test suite alongside functional tests
- **Para el DevOps Engineer**: Azure Static Web Apps preview environment requirements for accessibility scans
- **Para el Tech Writer**: pattern library and token map as the canonical source for UI documentation
- **Para el DevRel**: public-facing design system artifacts for developer audiences
