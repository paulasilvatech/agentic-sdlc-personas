...
> The QA Engineer es la persona que designs, runs, and evolves the test strategy for the whole product. En un SDLC AI-native, el QA Engineer operates a Test Strategist agent, a bank of slash prompts, and a validated MCP catalog — not a manual test console.

## Resumen ejecutivo

The QA Engineer owns the Verification phase. Their job is to prove that the code shipped by Developers actually satisfies the EARS requirements and Given-When-Then acceptance criteria locked into `SPECIFICATION.md`. En un SDLC AI-native, that proof is produced by a Test Strategist agent, cuatro slash prompts, instrucciones con alcance, and a small set of validated MCPs that reach into Azure DevOps Test Plans, GitHub Actions, and Playwright.
...
Piense en el QA Engineer like a flight-test engineer. The pilots fly, the mechanics build, but someone has to design the envelope-expansion plan that proves the aircraft safe across every regime it will ever see. En un SDLC AI-native, el QA Engineer is accountable for the envelope of automated and exploratory evidence that lets the team ship every day with confidence.
...
1. Como QA Engineer, quiero a generated test plan for every feature that links back to spec IDs, para que no acceptance criterion ships unverified.
2. Como QA Engineer, quiero mutation scores per module, para que I know where assertions are weak before the incident finds out.
3. Como QA Engineer, quiero flaky tests triaged within one working day, para que the signal stays trustworthy.
4. Como QA Engineer, quiero coverage gaps routed as issues with suggested tests, para que Developers close them in the same sprint.
5. Como QA Engineer, quiero Playwright runs recorded and indexed, para que post-incident analysis is a query, not an archaeology expedition.
6. Como QA Engineer, quiero the test matrix exported to Azure DevOps Test Plans, para que non-engineering stakeholders can browse verification evidence.
7. Como QA Engineer, quiero accessibility checks embedded in every end-to-end run, para que WCAG regressions are caught at merge time.
8. Como QA Engineer, quiero a weekly summary posted to Microsoft Teams, para que the whole org sees product quality trend lines.
...
- **L2 Asistido**: Copilot autocomplete inside Playwright specs, test matrix in a wiki, manual flake triage, ad-hoc exploratory sessions.
- **L3 Aumentado**: Test Strategist agent, cuatro slash prompts, instrucciones con alcance, Playwright MCP wired to GitHub Actions, mutation scans on demand.
- **L4 Autónomo**: Nightly mutation scan with auto-routed issues, flake triage closing root causes within 24 hours, requirements-to-tests traceability at 100 percent, verification dashboard published daily to Microsoft Teams.

## Integración con otras personas

- **Del Business Analyst**: fresh EARS requirements with unique IDs for traceability.
- **Del Developer**: merged PR with passing unit tests and a declared spec link.
- **Para el Developer**: coverage-gap and mutation-survivor issues with suggested test drafts.
- **Del Software Architect**: architectural risk register that informs resilience test lanes.
- **Para el SRE**: verified deployment artifact and updated synthetic probes.
- **Para el Compliance Auditor**: traceability matrix linking requirements to evidence of test execution.
