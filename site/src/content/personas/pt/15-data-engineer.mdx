...
> The Data Engineer é a persona que builds and operates the data pipelines the rest of the org depends on. Em um SDLC AI-native, o Data Engineer operates a Pipeline Keeper agent, quatro slash prompts, and a validated MCP catalog spanning Azure Data Factory, Synapse, Fabric, SQL Database, and Cosmos DB — not a forest of brittle notebooks.

## Resumo executivo

The Data Engineer owns the movement, transformation, and quality of data between systems. Em um SDLC AI-native, oir primary deliverables are idempotent pipelines, tested schema migrations, quality gates enforced in CI, and a lineage map that any stakeholder can read. The toolkit is fixed: the Pipeline Keeper agent, the `/etl-scaffold`, `/schema-diff`, `/quality-gate`, `/lineage-map` slash prompts, instruções escopadas for SQL and pipeline definitions, and validated MCPs reaching into Azure Data Factory, Azure Synapse, Microsoft Fabric, Azure SQL Database, and Azure Cosmos DB.
...
Pense no Data Engineer like the bridge master of a municipal water system. Pipes move water from reservoirs to neighborhoods; meters, valves, and testing stations ensure quality; any leak or contamination must be detected and routed before the kitchen tap. Em um SDLC AI-native, o Data Engineer owns that plumbing for facts instead of liquid.
...
1. Como Data Engineer, eu quero scaffold a new ETL pipeline in minutes, para que onboarding a new source is not a week-long ticket.
2. Como Data Engineer, eu quero every schema change reviewed with a diff and impact report, para que no silent breakage reaches production.
3. Como Data Engineer, eu quero a quality gate on every dataset, para que downstream consumers can trust freshness and completeness.
4. Como Data Engineer, eu quero a live lineage map, para que impact analysis for a schema change is a query, not a meeting.
5. Como Data Engineer, eu quero pipeline failures to produce actionable alerts in Azure Monitor, para que on-call work is prioritized correctly.
6. Como Data Engineer, eu quero cost anomalies in Synapse and Fabric detected automatically, para que runaway jobs do not blow the monthly budget.
7. Como Data Engineer, eu quero sensitive-data classifications from Microsoft Purview propagated into pipeline tests, para que PII handling is proven, not assumed.
8. Como Data Engineer, eu quero every transformation expressed in version-controlled code, para que no one-off SQL lives only in a notebook.
...
3. Implement transformations in SQL or PySpark with instruções escopadas enforcing style and safety.
...
- **L2 Assistido**: Copilot autocomplete in SQL, some pipelines in Azure Data Factory, but no schema diff and no quality gate.
- **L3 Aumentado**: Pipeline Keeper agent, quatro slash prompts, instruções escopadas, quality gate in CI, lineage refreshed weekly.
- **L4 Autônomo**: Quality gate blocking merges, cost anomalies opening issues within minutes, lineage refreshed on every merge, Purview classifications propagated into tests.

## Integração com outras personas

- **Do Business Analyst**: data requirements mapped to EARS, source-system contracts.
- **Do Software Architect**: storage and throughput constraints, target platform selection.
- **With DBA**: shared schema governance for operational stores; migration reviews.
- **Para o ML AI Engineer**: curated feature datasets in Microsoft Fabric with a documented lineage.
- **Para o BI Analyst**: certified datasets with freshness and completeness guarantees.
- **Para o SRE**: Azure Monitor runbooks for pipeline failures.
- **Para o Compliance Auditor**: Purview classifications, lineage map, and quality-gate history as audit evidence.
