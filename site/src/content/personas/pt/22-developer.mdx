...
- `dep-risk-scan`: chama o Snyk MCP em toda atualização de dependência
...
| [Sentry MCP Server](https://github.com/getsentry/sentry-mcp) | Oficial | Puxar o contexto do erro para o loop de fix-bug |
| [Snyk MCP Server](https://docs.snyk.io/scan-with-snyk/snyk-mcp) | Oficial | Verificar vulnerabilidades em upgrade de dependência e durante refactor |
| [Linear MCP Server](https://linear.app/docs/mcp) | Oficial | Ler o ticket ativo, atualizar a issue após merge do PR |
| [Figma Dev Mode MCP](https://developers.figma.com/docs/figma-mcp-server/) | Oficial | Puxar design tokens e specs de componentes ao implementar UI |
...
**Entrada**: Um alerta do Sentry reporta null pointer em `ContractService.findById` disparado por requisições concorrentes.
...
3. Um PR intitulado `fix(contracts): eliminate race in ContractService.findById` linkado à issue do Sentry e ao teste novo.
4. Uma resolução pós-merge da issue do Sentry com a URL do PR registrada.
...
| Taxa de falha de mudança | 20 por cento | < 5 por cento | Sentry ou incidentes pós-deploy |
...
- **MCP**: servidor Model Context Protocol que expõe sistemas externos (GitHub, Sentry, Linear, etc.) ao agente.
