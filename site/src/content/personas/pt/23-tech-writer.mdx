...
> The Tech Writer is the persona accountable for documentation that stays current, navigable, and trustworthy. Em um SDLC AI-native, o Tech Writer operates a docs-as-code pipeline, not a knowledge-base portal.

## Resumo executivo

The Tech Writer makes the system legible to humans and to AI agents. Em um SDLC AI-native, o Tech Writer works inside the Delivery phase com um conjunto fixo de primitivas: um agente docs-curator, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. The primary outputs are MDX and Markdown content in the repository, generated API references, release notes, and a curated table of contents that ships as a static site on Azure Static Web Apps.

## Papel e responsabilidades

Pense no Tech Writer like the map-maker in a growing city. Streets and buildings appear every week; without the map-maker, newcomers are lost and longtimers forget which street is which. Em um SDLC AI-native, o code and architecture change daily; without the Tech Writer, the map is two quarters stale and the AI agents hallucinate landmarks that no longer exist.
...
1. Como Tech Writer, eu quero an approved specification converted into a draft doc page in minutes, para que I ship docs with features, not after them.
2. Como Tech Writer, eu quero the style guide enforced automatically in CI, para que I coach instead of police.
3. Como Tech Writer, eu quero the table of contents refreshed whenever a new page lands, para que navigation stays coherent as the site grows.
4. Como Tech Writer, eu quero release notes drafted from merged PRs and linked issues, para que every release has a human-readable summary.
5. Como Tech Writer, eu quero detect stale docs automatically when the underlying code changes, para que deprecated guidance disappears before users stumble into it.
6. Como Tech Writer, eu quero AI agents to read the docs correctly, para que in-product assistants give users accurate answers.
...
The Tech Writer operates a daily loop. O loop usa primitivas do GitHub Copilot inside Visual Studio Code, Claude Code at the terminal for long-form drafting, and the Microsoft Learn Docs MCP for grounded references on Microsoft and Azure stacks.

### Setup da manhã

1. Open Visual Studio Code on the `docs` repository. GitHub Copilot Chat carrega the scoped `.github/instructions/docs.instructions.md`.
...
1. **Copy-paste release notes from commits**. Commit messages are for engineers; release notes are for users. Mitigação: `release-notes.instructions.md` requires user-centric phrasing and rewrites jargon.
2. **Docs that live outside the repository**. A knowledge-base portal detached from code drifts instantly. Mitigação: every page lives in `docs/` and ships via GitHub Actions to Azure Static Web Apps.
3. **Screenshots that age silently**. Image-based UI docs rot fastest. Mitigação: Playwright MCP regenerates critical screenshots on every docs build.
4. **ToC by hand**. A hand-maintained navigation tree goes stale within a sprint. Mitigação: `/toc-refresh` and the post-commit hook keep the JSON generated.
5. **Style policing in PR reviews**. Manual policing frustrates both sides. Mitigação: `/style-check` runs in CI and comments inline.
...
| L2 | Assistido | GitHub Copilot Chat for drafting, docs in repo, ad hoc CI checks |
| L3 | Aumentado | Docs Curator agent, quatro slash prompts, instruções escopadas, style and link checks in GitHub Actions, site on Azure Static Web Apps |
