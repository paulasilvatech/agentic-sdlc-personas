...
> The Product Owner é a persona que writes, governs, and closes the loop on the specification. Em um SDLC AI-native, o Product Owner opera uma stack de primitivas validadas that turn intent into a machine-readable contract.

## Resumo executivo

The Product Owner converts ambiguous business intent into an approved, testable specification that the rest of the SDLC can execute without translation loss. Em um SDLC AI-native, o Product Owner opera dentro da fase de Planning phase com um conjunto fixo de primitivas: um agente specification, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. Saídas principais são `SPECIFICATION.md` documents in EARS form, acceptance criteria in Given-When-Then, traceability links from requirement to test, and decision records for every scope change.

## Papel e responsabilidades

Pense no Product Owner like a civil engineer authoring the specification of a bridge. The builders, inspectors, and regulators all read the same document and derive their work from it. The Product Owner does not pour concrete, but they are accountable for the fact that every pour, weld, and cable matches a numbered clause that someum agente can verify under load. Em um SDLC AI-native, o specification is the contract between humans ands, and the Product Owner is its editor in chief.
...
1. Como Product Owner, eu quero convert a stakeholder conversation into a draft spec within one hour, para que the team never waits on me to start discovery.
2. Como Product Owner, eu quero every requirement in EARS form, para que agents can parse it without interpretation.
3. Como Product Owner, eu quero every acceptance criterion in Given-When-Then, para que the QA Engineer can generate tests directly from the spec.
4. Como Product Owner, eu quero a live traceability link from each requirement to its test and its deployed artifact, para que I can answer audit questions in minutes.
5. Como Product Owner, eu quero the spec to refuse merge when acceptance is missing, para que scope drift is caught before code is written.
6. Como Product Owner, eu quero release notes to be generated from the spec diff, para que stakeholders see value delivered, not commits shipped.
...
The Product Owner operates a fixed loop each day. O loop usa primitivas do GitHub Copilot dentro do Visual Studio Code e Claude Code no terminal, além de um pequeno catálogo de MCPs validados para contexto externo.

### Setup da manhã

1. Abrir o repositório no Visual Studio Code. GitHub Copilot Chat carrega `AGENTS.md` and the scoped spec instructions.
2. Fazer pull da última `main` and open the active feature branch for the spec in progress.
...
2. Abrir um pull request on `SPECIFICATION.md`. GitHub Copilot Code Review comments on structural issues; human stakeholders approve content.
...
Hooks custam zero tokens de LLM. Eles são a camada de governança mais forte for specifications.
...
**Invocação**: `/draft-spec` with the thread exported via the Microsoft 365 Agents SDK MCP.
...
1. **Writing specs as prose**. Paragraphs hide contradictions and cannot be consumed by agents. Mitigação: enforce EARS via the `pre-commit` hook and the `/earsify` prompt.
2. **Skipping acceptance criteria**. A requirement without Given-When-Then is not testable. Mitigação: `/link-acceptance` refuses to close the loop, and the hook blocks commit.
3. **Backlog items without spec anchors**. If the ticket does not link to a numbered requirement, scope will drift. Mitigação: the GitHub MCP auto-injects the anchor URL on issue creation.
4. **Release notes generated from commits**. Stakeholders see technical noise. Mitigação: release notes are generated from the spec diff via the instruções escopadas.
5. **Ambiguous verbs**. Words like "support", "handle", "improve" are banned by the spec instructions. Mitigação: the `pre-commit` hook rejects them.
...
| L2 | Assistido | GitHub Copilot used to polish ticket text, still no machine-readable spec |
| L3 | Aumentado | Spec Scribe agent, quatro slash prompts, instruções escopadas, one or two MCPs, EARS enforced |
| L4 | Autônomo | Full primitives kit, hooks enforced, traceability auto-generated, release notes from spec diff, stakeholder digest automated |

## Integração com outras personas

- **Para o Requirements Engineer**: approved `SPECIFICATION.md` with numbered EARS requirements as the canonical input for detailed decomposition
- **Para o Business Manager**: requirement-level KPI hooks so outcomes can be tied to the value story
- **Para o Enterprise Architect**: spec clauses that trigger a new ADR or invoke an existing principle
- **Para o Software Architect**: contract surface area used to derive `CODEMAP.md` and API contracts
- **Para o QA Engineer**: Given-When-Then acceptance as the direct source of test cases
- **Para o Developer**: spec anchor on every pull request for traceable implementation
- **Para o Tech Writer**: release notes generated from the spec diff, not the commit diff
...
- [Azure Boards documentation](https://learn.microsoft.com/azure/devops/boards/) — rastreamento de work items when the team uses Azure DevOps
