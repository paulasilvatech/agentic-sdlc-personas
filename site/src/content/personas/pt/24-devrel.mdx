...
> The DevRel is the persona accountable for the developer experience outside the core team: tutorials, demo repositories, community conversations, and feedback loops back into the product. Em um SDLC AI-native, DevRel operates a content and community pipeline, not an event calendar.

## Resumo executivo

DevRel turns product capability into community proficiency. Em um SDLC AI-native, DevRel works inside the Community phase com um conjunto fixo de primitivas: um agente demo-builder, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. The primary outputs are reproducible demo repositories on GitHub, tutorials published through Microsoft Learn-style content, video briefs for recorded sessions, and a regular community digest that feeds both external readers and the product team.

## Papel e responsabilidades

Pense no DevRel like a concert promoter and a session musician rolled into one. The promoter books the venue and warms up the crowd; the session musician can sit in with the band and make the music sound better. Em um SDLC AI-native DevRel both amplifies the product and plays alongside developers who adopt it.
...
1. Como DevRel, eu quero a demo repository scaffolded in minutes from a chosen scenario, para que I record a session the same afternoon.
2. Como DevRel, eu quero tutorials that cite current Microsoft Learn pages, para que readers never follow outdated steps.
3. Como DevRel, eu quero a weekly digest of GitHub Discussions themes, para que the product team hears what the community is actually asking.
4. Como DevRel, eu quero video briefs with time-boxed sections and reproducible code snippets, para que recordings stay tight and editable.
5. Como DevRel, eu quero every demo repository covered by GitHub Actions CI, para que viewers can fork and run without hitting stale dependencies.
6. Como DevRel, eu quero community contributions acknowledged systematically, para que the best contributors stay engaged.
...
The DevRel operates a daily loop. O loop usa primitivas do GitHub Copilot inside Visual Studio Code, Claude Code at the terminal for long-form scripting, and the Microsoft Learn Docs MCP for grounded tutorials on Azure and Microsoft 365 topics.

### Setup da manhã

1. Open Visual Studio Code on the `devrel` repository. GitHub Copilot Chat carrega the scoped `.github/instructions/devrel.instructions.md`.
...
1. **Demos without CI**. A demo that is not built by GitHub Actions breaks silently. Mitigação: the pre-push hook rejects demos with no green workflow in the last seven days.
2. **Tutorials grounded in memory**. Writing from memory is a drift generator. Mitigação: `tutorials.instructions.md` requires Microsoft Learn citations via the Microsoft Learn Docs MCP.
3. **Community feedback as anecdotes**. Sharing one or two loud quotes is not a signal. Mitigação: `/community-digest` clusters themes with representative links.
4. **Video sessions with no brief**. An unbriefed session runs long and edits poorly. Mitigação: `/video-brief` enforces time-box and on-screen cue discipline.
5. **Contribution recognition ad hoc**. Ad hoc thank-you posts miss most contributors. Mitigação: the digest section on contributors pulls merged PRs and accepted Discussions answers from the GitHub MCP.
...
| L2 | Assistido | GitHub Copilot Chat for drafting, some demos on GitHub Actions, inconsistent citations |
| L3 | Aumentado | Demo Builder agent, quatro slash prompts, instruções escopadas, Microsoft Learn Docs MCP grounding, Playwright smoke tests |
...
- [GitHub Actions documentation](https://docs.github.com/actions) — CI and deployment automação para demo repositories
