...
> The InfoSec Officer é a persona que keeps software and AI systems defensible under change. Em um SDLC AI-native, o InfoSec Officer operates a Threat Triager agent, quatro slash prompts, and a validated MCP catalog anchored on GitHub Advanced Security and Microsoft Defender for Cloud — not a backlog of PDF checklists.

## Resumo executivo

The InfoSec Officer owns the security posture of the delivery pipeline and the products it ships. Em um SDLC AI-native, oy operate a Threat Triager agent with quatro slash prompts (`/vuln-triage`, `/sbom-scan`, `/threat-model`, `/incident-security`), instruções escopadas for security-sensitive paths, and a validated MCP catalog that reaches into GitHub Advanced Security, Dependabot, CodeQL, Secret Scanning, Push Protection, Microsoft Defender for Cloud, Microsoft Sentinel, Microsoft Purview, Entra ID, and Azure Key Vault.
...
Pense no InfoSec Officer like the fire marshal for a city. They do not fight every fire, but they write the building code, certify inspections, run drills, and coordinate the response when a real fire happens. Em um SDLC AI-native, o InfoSec Officer enforces the code and orchestrates the response across GitHub, Azure, and Microsoft 365 surfaces.
...
- **L2 Assistido**: Dependabot and Secret Scanning enabled but untriaged; Defender for Cloud dashboards watched ad hoc.
- **L3 Aumentado**: Threat Triager agent, quatro slash prompts, instruções escopadas, CodeQL custom pack, Sentinel integration.
- **L4 Autônomo**: Automated triage with SLA enforcement, signed SBOMs blocking release, threat models attached to every architecture PR, incidents with auto-generated timelines.

## Integração com outras personas

- **Do Software Architect**: design diagrams and ADRs feeding `/threat-model`.
- **Para o Developer**: remediation issues with linked fix drafts.
...
- **Do Data Engineer**: Purview classifications driving data-handling controls.
