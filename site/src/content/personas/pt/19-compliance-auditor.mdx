...
> The Compliance Auditor é a persona que converts regulation into enforceable controls and verifiable evidence. Em um SDLC AI-native, o Compliance Auditor operates an Evidence Curator agent, quatro slash prompts, and a validated MCP catalog spanning GitHub, Microsoft Purview, and Azure Policy — not a library of static attestation PDFs.

## Resumo executivo

The Compliance Auditor owns SOX, ISO 27001, SOC 2, and sector-specific obligations for delivery. Em um SDLC AI-native, o role is operationalized through a single Evidence Curator agent with quatro slash prompts (`/control-check`, `/audit-pack`, `/policy-diff`, `/attestation-draft`), instruções escopadas for policy and control files, and validated MCPs reaching into GitHub, Microsoft Purview, Azure Policy, and Microsoft Sentinel.
...
Pense no Compliance Auditor like a building inspector who signs off construction stage by stage instead of just at the end. They read the blueprints, verify every pour, every wire, every fixture — and when the final walkthrough happens, there are no surprises. Em um SDLC AI-native, o Compliance Auditor inspects the pipeline at every stage by inspecting its artifacts.
...
1. Como Compliance Auditor, eu quero every control mapped to a live check, para que "compliant" is a continuous state, not a moment.
2. Como Compliance Auditor, eu quero evidence packs assembled in minutes, para que external auditors receive what they need same-day.
3. Como Compliance Auditor, eu quero policy changes reviewed as PRs, para que the trail of who-approved-what is verifiable.
4. Como Compliance Auditor, eu quero attestations drafted from actual pipeline evidence, para que signatures reflect reality.
5. Como Compliance Auditor, eu quero exceptions time-boxed and tracked, para que no exception becomes permanent by neglect.
6. Como Compliance Auditor, eu quero Microsoft Purview sensitivity labels enforced in code repos, para que data handling obligations are propagated.
7. Como Compliance Auditor, eu quero Entra ID access reviews scheduled automatically, para que least-privilege reviews never lapse.
8. Como Compliance Auditor, eu quero a single posture dashboard published to Microsoft Teams, para que leadership has a current view without asking.
...
- **L2 Assistido**: Some controls wired to Defender for Cloud, but evidence still assembled manually at quarter end.
- **L3 Aumentado**: Evidence Curator agent, quatro slash prompts, instruções escopadas, audit packs in hours.
- **L4 Autônomo**: Continuous control checks, daily posture score, expiring exceptions auto-closed, attestations drafted from evidence.

## Integração com outras personas

- **With InfoSec Officer**: shared controls, joint SBOM and threat-model evidence.
- **Do Software Architect**: architecture ADRs mapped to controls in the catalog.
- **Do Developer**: PR metadata feeding change-management evidence.
- **Do Data Engineer**: Microsoft Purview classifications driving data-handling evidence.
- **Do SRE**: incident timelines and runbooks as operational-resilience evidence.
- **With Legal**: regulatory interpretation and policy text changes.
- **Para o Product Owner**: compliance status on features ahead of release.
