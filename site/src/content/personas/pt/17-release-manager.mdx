...
1. Como Release Manager, eu quero release notes generated from merged PRs, para que cut day is a signoff, not a scramble.
2. Como Release Manager, eu quero every release to carry a risk tier and a rollback plan, para que go or no-go is a minutes-long meeting.
3. Como Release Manager, eu quero canary signals read by an agent, para que human attention is spent on ambiguous cases only.
4. Como Release Manager, eu quero CHANGELOG entries versioned alongside the code, para que downstream consumers never diff by hand.
5. Como Release Manager, eu quero release communication sent to Teams and SharePoint automatically, para que the release channel is never the bottleneck.
6. Como Release Manager, eu quero postmortems to feed forward into future release gates, para que the same incident never ships again.
...
The Release Manager operates a fixed loop each release cycle. O loop usa primitivas do GitHub Copilot dentro do Visual Studio Code e Claude Code no terminal, além de um pequeno catálogo de MCPs validados para contexto externo.

### Setup da manhã

1. Open the release coordination repo in Visual Studio Code. GitHub Copilot Chat carrega `AGENTS.md` and the scoped `.github/instructions/release.instructions.md`.
...
| [Azure MCP Server](https://github.com/Azure/azure-mcp) | Official (Microsoft) | Consultar Application Insights, Azure Monitor alerts, and Azure Policy compliance |
...
**Entrada**: 42 PRs have merged across 7 services since the previous release tag `v2.14.0`.

**Invocação**: `/release-notes` followed by `/risk-gate`.
...
**Entrada**: The canary of `v2.15.0` has been at 5 percent traffic for 30 minutes. Latency p95 is up 8 percent; error rate is within noise.
...
1. **Cut-day release notes**. Notes written the night before with scraped commits. Mitigação: `/release-notes` composes continuously from merged PRs, not at cut.
2. **Risk tier by feel**. Reviewers approve without a reproducible heuristic. Mitigação: `risk-heuristic` skill produces tier from PR size, blast radius, and security alert deltas.
3. **Rollback plan as "redeploy previous tag"**. No flag inventory, no data reversal. Mitigação: `/rollback-plan` produces the complete sequence; rehearsed in staging before release.
4. **Canary read by staring**. A human watches dashboards for 45 minutes. Mitigação: `/canary-report` reads Application Insights programmatically.
5. **Release communication by email forward**. Stakeholders learn about the release when alerts fire. Mitigação: Microsoft 365 Agents SDK MCP posts the train departure and arrival automatically.
...
| L2 | Assistido | GitHub Copilot helps draft notes, CHANGELOG maintained, no agent |
| L3 | Aumentado | One Release Scribe agent, quatro slash prompts, instruções escopadas, two MCPs, canary read semi-automated |
...
- **Do DevOps Engineer**: release train candidate, risk tiers, what-if diffs
- **Do QA Engineer**: test matrix, regression report, known issues
- **Para o SRE**: deployed build, canary window, alert posture
- **Para o Tech Writer**: release notes, CHANGELOG, migration guides
- **Para o Product Owner**: stakeholder-facing release summary posted to SharePoint
