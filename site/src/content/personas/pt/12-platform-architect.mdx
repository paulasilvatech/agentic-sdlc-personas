...
1. Como Platform Architect, eu quero a new service repo created from a golden path template in minutes, para que teams start on the paved road.
2. Como Platform Architect, eu quero every service to declare its capabilities in a machine-readable matrix, para que platform evolution is data-driven.
3. Como Platform Architect, eu quero ADRs to be drafted from design conversations, para que the decision record is never skipped.
4. Como Platform Architect, eu quero templates versioned and rolled forward via automated PRs, para que the paved road stays paved.
5. Como Platform Architect, eu quero platform usage telemetry to flow into Application Insights, para que unused capabilities are retired, not accumulated.
6. Como Platform Architect, eu quero the MCP catalog to be enforced at commit time, para que teams cannot install rogue MCPs.
...
The Platform Architect operates a fixed loop each day. O loop usa primitivas do GitHub Copilot dentro do Visual Studio Code e Claude Code no terminal, além de um pequeno catálogo de MCPs validados para contexto externo.

### Setup da manhã

1. Open the platform monorepo in Visual Studio Code. GitHub Copilot Chat carrega `AGENTS.md` and the scoped `.github/instructions/*.instructions.md` for templates and ADRs.
...
**Invocação**: `/golden-path` with archetype `internal-api`.
...
**Invocação**: `/template-new` with the template version bump.
...
1. **Template as wiki page**. A markdown page that describes the golden path without a scaffolding engine. Mitigação: every golden path is a real template repo with parameters and tests.
2. **ADRs written after the fact**. Decisions are documented months later, if at all. Mitigação: `/adr-platform` drafts from the design meeting transcript during the meeting.
3. **Manual capability matrix**. A spreadsheet that nobody updates. Mitigação: `/capability-matrix` regenerates from the catalog YAML.
4. **MCP free-for-all**. Teams install any MCP they find. Mitigação: `mcp-catalog-enforcer` skill refuses PRs that reference uncatalogued MCPs.
5. **Policy per subscription**. Each subscription grows its own Azure Policy tree. Mitigação: a single initiative owned by the Platform Architect, assigned at the management group.
...
| L2 | Assistido | Template repo exists but drifts, GitHub Copilot helps draft ADRs occasionally |
| L3 | Aumentado | One Path Keeper agent, quatro slash prompts, instruções escopadas, two MCPs, template rollout automated |
...
- **Do Enterprise Architect**: target architecture, reference patterns, investment themes
- **Do Software Architect**: service-level ADRs that bubble up to platform decisions
- **Para o DevOps Engineer**: reusable workflows, Bicep modules, policy initiatives
- **Para o Developer**: scaffolded repo, instruções escopadas, validated MCP catalog
- **Para o InfoSec Officer**: policy initiative, MCP catalog, Entra ID app registration skeleton
