...
> The Business Manager é a persona que binds strategy to measurement. Em um SDLC AI-native, o Business Manager opera uma stack de primitivas validadas that turn business intent into machine-readable KPIs and outcome hypotheses.

## Resumo executivo

The Business Manager translates enterprise strategy and OKRs into product-level KPIs, value stories, and investment cases that the rest of the SDLC can optimize against. Em um SDLC AI-native, o Business Manager opera dentro da fase de Planning phase com um conjunto fixo de primitivas: um agente KPI translation, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. Saídas principais são OKR trees, KPI specifications, business cases, and value stories linked to spec anchors.

## Papel e responsabilidades

Pense no Business Manager like the conductor of an orchestra reading the score the composer wrote. The conductor does not play any instrument, but they are accountable for the fact that every section plays in time and on key, and that what the audience hears matches the composer's intent. Em um SDLC AI-native, o OKR tree is the score, the KPI specs are the tempo markings, and the Business Manager is responsible for the performance across product, architecture, and build.
...
1. Como Business Manager, eu quero convert a strategic objective into an OKR tree within a day, para que the portfolio is aligned at the start of every quarter.
2. Como Business Manager, eu quero every KPI to have a baseline, target, and measurement method, para que outcomes are auditable, not anecdotal.
3. Como Business Manager, eu quero the value story to be linked to the spec, para que engineering decisions trace back to business intent.
4. Como Business Manager, eu quero business cases generated from templated inputs, para que the cycle time from idea to funding is measured in days, not weeks.
5. Como Business Manager, eu quero live telemetry on KPI achievement from Application Insights, para que I can intervene before the quarter ends.
6. Como Business Manager, eu quero a monthly portfolio health report generated automatically, para que leadership decisions are grounded in current data.
...
The Business Manager operates a fixed loop each day. O loop usa primitivas do GitHub Copilot dentro do Visual Studio Code e Claude Code no terminal, além de um pequeno catálogo de MCPs validados para contexto externo.

### Setup da manhã

1. Open the portfolio repository in Visual Studio Code. GitHub Copilot Chat carrega `AGENTS.md` and the scoped OKR instructions.
...
2. Abrir um pull request on the OKR tree. GitHub Copilot Code Review comments on measurability; leadership reviewers approve content.
...
Hooks custam zero tokens de LLM. Eles são a camada de governança mais forte for business artifacts.
...
**Invocação**: `/okrs` with the theme and last year's telemetry pulled from the Azure MCP.
...
**Invocação**: `/biz-case` followed by `/value-story`.
...
1. **KRs without baselines**. A target of "increase adoption" is unmeasurable. Mitigação: the `pre-commit` hook rejects any KR without a baseline and source.
2. **Slide-first OKRs**. If the authoritative copy lives in a deck, it cannot be queried or diffed. Mitigação: the OKR tree is authored in markdown under version control.
3. **Value stories disconnected from specs**. When the narrative does not link to a spec anchor, engineering optimizes something else. Mitigação: `/value-story` refuses to close without a spec link.
4. **Telemetry ignored post-launch**. Declaring success on ship date is a habit, not a measurement. Mitigação: the portfolio dashboard flags any KR with stale telemetry.
5. **Business cases in spreadsheets**. Static files drift from reality. Mitigação: business cases live in markdown with hooks that validate template completeness.
...
| L2 | Assistido | Copilot used to polish OKR prose, still no machine-readable artifacts |
| L3 | Aumentado | KPI Translator agent, quatro slash prompts, instruções escopadas, Azure MCP for telemetry, KPIs with baselines |
| L4 | Autônomo | Full primitives kit, hooks enforced, portfolio dashboard live, leadership digest automated, post-launch verification standard |

## Integração com outras personas

- **Do Enterprise Architect**: capability scan and constitution principles that constrain the OKR tree
- **Para o Product Owner**: approved OKRs and KPIs that the spec must hook into via `/link-acceptance`
- **Para o Requirements Engineer**: KPI-bound outcomes that inform gap scans and traceability
- **Para o Engineering Manager**: OKR tree that drives capacity planning and squad assignment
- **Para o SRE**: SLOs aligned with customer-facing KPIs for reliability investment
- **Para o Release Manager**: value story as the backbone of release narrative and stakeholder comms
