...
> The Technical Lead é a persona que engineers context for the squad. Em um SDLC AI-native, o Technical Lead opera uma stack de primitivas validadas that make the team's agents fast, cheap, and correct.

## Resumo executivo

The Technical Lead owns the team-level primitives catalog: `AGENTS.md`, instruções escopadas, prompts, skills, hooks, and the routing table that binds them. Em um SDLC AI-native, o Technical Lead opera dentro da fase de Design phase com um conjunto fixo de primitivas: um agente context auditing, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. Saídas principais são the audited context budget, the scope-split plan for large tasks, primed context packs for new features, and the squad primitives kit.

## Papel e responsabilidades

Pense no Technical Lead like the chief of staff for a surgical team. They do not hold the scalpel, but they make sure the instruments are sterile, labeled, and within reach before the surgeon starts. Em um SDLC AI-native, o instruments are agents, prompts, instructions, skills, and hooks, and the Technical Lead is accountable for the fact that the squad reaches for the right tool without reading the whole repository every time.

Responsabilidades principais:

- Author and maintain `AGENTS.md`, the root prompt for every agent in the repository
- Curate the squad's instruções escopadas to keep the context budget low
...
1. Como Technical Lead, eu quero the squad's context budget to stay under threshold, para que agents are fast and affordable.
2. Como Technical Lead, eu quero oversized tasks split into parallelizable units, para que the squad delivers in hours, not sprints.
3. Como Technical Lead, eu quero context primed before a Developer starts a feature, para que Copilot and Claude Code find the right files immediately.
4. Como Technical Lead, eu quero the squad primitives kit to be a single directory, para que onboarding a new engineer costs minutes.
5. Como Technical Lead, eu quero instruções escopadas to carry `applyTo` patterns, para que token cost scales with the diff, not the repo.
6. Como Technical Lead, eu quero context audits to run on every PR, para que the catalog does not rot.
...
4. **Large tasks stuck in um agente session**. A feature meant for three engineers takes one three days because nobody split the scope.
...
The Technical Lead operates a fixed loop each day. O loop usa primitivas do GitHub Copilot dentro do Visual Studio Code e Claude Code no terminal, além de um pequeno catálogo de MCPs validados para contexto externo.

### Setup da manhã

1. Open the squad repository in Visual Studio Code. GitHub Copilot Chat carrega `AGENTS.md` and the scoped primitives instructions.
2. Fazer pull da última `main` and list prompts, instructions, agents, and hooks changed overnight.
...
2. **Scope splitting**. Invoke `/scope-split` on any task estimated above um agente working day. The decomposes the work into parallelizable units with explicit dependencies and handoffs.
...
2. Abrir um pull request on primitives changes. GitHub Copilot Code Review comments on `applyTo` patterns, duplication, and token-cost impact.
...
Hooks custam zero tokens de LLM. Eles são a camada de governança mais forte for primitives.
...
| [Azure DevOps MCP Server](https://github.com/microsoft/azure-devops-mcp) | Ler work items do Azure Boards that inform scope splits when the team uses Azure DevOps | Microsoft (official) |
...
1. **Growing `AGENTS.md` by accretion**. Every squad adds a paragraph, nobody removes one. Mitigação: `pre-commit` hook enforces the root budget.
2. **Copy-paste prompts across repos**. Drift guarantees inconsistent behavior. Mitigação: central prompt catalog with semantic versioning and `/team-primitives` curation.
3. **Eagerly loading the whole `docs/`**. Token waste. Mitigação: instruções escopadas with `applyTo` and a primed-context pack per feature.
4. **Splitting scope without dependencies**. Parallel units that silently collide produce rework. Mitigação: `/scope-split` emits an explicit dependency graph and enforces it in the routing table.
5. **Primitives authored without review**. Unvetted prompts invite hallucination. Mitigação: every primitive lands via PR with Copilot Code Review and human approval.
...
| L2 | Assistido | `AGENTS.md` exists but untested, no instruções escopadas, no audits |
| L3 | Aumentado | Context Auditor agent, quatro slash prompts, instruções escopadas, GitHub MCP, audit-context on PR |
| L4 | Autônomo | Full primitives kit, hooks enforced, primed-context standard per feature, scope splits automatic, routing table live |

## Integração com outras personas

- **Do Enterprise Architect**: principles and gate expressions that constrain primitives design
- **Do Software Architect**: `CODEMAP.md` and contracts that inform the routing table
- **Para o Developer**: primed-context packs, scope splits, and curated prompts ready for use
- **Para o QA Engineer**: testable scope units with explicit acceptance linking
- **Para o DevOps Engineer**: hooks as the governance layer on every workflow
- **Para o Engineering Manager**: audit data that grounds capacity and staffing decisions
- **Para o Release Manager**: routing table as the canonical map from PRs to the primitives that produced them
...
- [GitHub Actions documentation](https://docs.github.com/en/actions) — automação para context audits and primitive governance
