...
> The Engineering Manager is the persona accountable for the health of the people and the health of the delivery flow. Em um SDLC AI-native, o Engineering Manager operates a governance loop over DORA signals, 1:1 agendas, and team-level retrospectives, not a spreadsheet of tickets.

## Resumo executivo

The Engineering Manager ensures that engineers grow, that delivery flow is predictable, and that organizational risk is surfaced before it becomes incident. Em um SDLC AI-native, o Engineering Manager works inside the Governance phase com um conjunto fixo de primitivas: um agente delivery-health, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. The primary outputs are DORA scorecards, 1:1 agendas, retrospective briefs, and staffing recommendations tied to measured flow.

## Papel e responsabilidades

Pense no Engineering Manager like the conductor of an orchestra during rehearsal season. The conductor does not play an instrument on stage, but the orchestra falls apart without their sense of tempo, dynamics, and rehearsal discipline. Em um SDLC AI-native, o code and the architecture are owned by other personas. The Engineering Manager is accountable for tempo: the cadence at which the team learns, ships, and recovers.
...
The Engineering Manager operates a weekly and daily loop. O loop usa primitivas do GitHub Copilot inside Visual Studio Code, Claude Code at the terminal for report generation, and the M365 Agents SDK on Microsoft Teams for 1:1 surfacing.

### Setup da manhã

1. Open Visual Studio Code on the `eng-ops` repository. GitHub Copilot Chat carrega the scoped `.github/instructions/management.instructions.md`.
...
1. Lead a retro for um agente of the teams using `/retro`. The ingests sprint data from Azure Boards and GitHub Projects and produces a structured brief: what worked, what stalled, systemic causes, proposed experiments.
...
O escopo `applyTo` reduz o custo de tokens and keeps people-facing content distinct from code review guidance.
...
| [Azure DevOps MCP Server](https://github.com/microsoft/azure-devops-mcp) | Official (Microsoft) | Ler work items do Azure Boards, iterations, and risk register entries |
...
1. **Turning 1:1 data into performance evidence**. The 1:1 agenda is a conversation scaffold, not an audit trail. Mitigação: the `one-on-ones.instructions.md` forbids verdict phrasing and requires opt-in sharing.
2. **Running DORA without the engineers seeing it first**. Metrics weaponized in leadership decks before the team sees them kill trust. Mitigação: the pulse posts to the team channel before the leadership channel.
3. **Retros that name individuals**. Blaming people is a management failure. Mitigação: the Delivery Pulse agent rewrites any individual-blame phrasing into systemic-cause phrasing.
4. **Staffing models that hide assumptions**. Capacity divided by story points is a lie. Mitigação: `/staff-ops` returns explicit assumptions and flags each one.
5. **Dashboards that never drive action**. A workbook nobody reads is noise. Mitigação: every anomaly on the pulse opens an Azure Boards work item with an owner.
...
| L2 | Assistido | GitHub Copilot Chat for drafting, no agent, dashboards in one tool only |
| L3 | Aumentado | Delivery Pulse agent, quatro slash prompts, instruções escopadas, DORA dashboard in Azure Workbooks |
