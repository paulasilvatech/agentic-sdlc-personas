...
> The DBA é a persona que keeps the operational data stores fast, safe, and honest under change. Em um SDLC AI-native, o DBA operates a Schema Guard agent, quatro slash prompts, and a validated MCP catalog across Azure SQL Database and Azure Cosmos DB — not a laptop full of SQL scripts.

## Resumo executivo

The DBA owns the health of operational databases — primarily Azure SQL Database and Azure Cosmos DB — across schema evolution, query performance, and reliability. Em um SDLC AI-native, oir workflow is operationalized through a Schema Guard agent with quatro slash prompts (`/migrate-plan`, `/index-review`, `/query-tune`, `/rollback-script`), instruções escopadas for SQL and migration files, and validated MCPs reaching into Azure Monitor, Application Insights, and GitHub.
...
Pense no DBA like the chief engineer of a ship. The captain sets course; the chief engineer ensures the engines never stall, the pumps keep running, and every fuel line is tested before the storm. Em um SDLC AI-native, o DBA keeps the data engines running under constant change.
...
1. Como DBA, eu quero every migration reviewed with a generated rollback script, para que no forward step is irreversible.
2. Como DBA, eu quero index suggestions surfaced from real workload telemetry, para que optimization work targets actual pain.
3. Como DBA, eu quero long-running queries tuned with agent-assisted analysis, para que p99 latencies stay within SLA.
4. Como DBA, eu quero Cosmos DB partition keys reviewed on data-model PRs, para que hot partitions do not appear at scale.
5. Como DBA, eu quero schema changes deployed behind feature flags with safe backfill, para que no release is gated by a DDL operation.
6. Como DBA, eu quero daily integrity checks and restore drills evidenced in Azure Monitor, para que resilience claims are verifiable.
7. Como DBA, eu quero database access reviews run with Entra ID, para que least-privilege is the default state.
8. Como DBA, eu quero a single schema-change log published to Microsoft Teams, para que product and engineering see what changed in shared data.
...
- **L2 Assistido**: Copilot drafts SQL, migrations tracked in a tool, but rollback is still ad hoc.
- **L3 Aumentado**: Schema Guard agent, quatro slash prompts, instruções escopadas, feature-flagged migrations.
- **L4 Autônomo**: Nightly index review, auto-validated rollbacks, restore drills scheduled and evidenced, daily schema-change log.

## Integração com outras personas

- **With Data Engineer**: shared schema governance for cross-store datasets; migration review.
- **Do Developer**: entity and ORM changes coordinated through PRs.
- **Do Software Architect**: storage technology choices and capacity models.
...
- **Para o Compliance Auditor**: evidence of change control, access reviews, backup posture.
