...
> The Project Manager is the persona accountable for predictable delivery across teams and for honest stakeholder communication. Em um SDLC AI-native, o Project Manager operates a risk and status loop, not a status-meeting calendar.

## Resumo executivo

The Project Manager converts a portfolio of commitments into a single, honest picture of progress, risk, and stakeholder expectations. Em um SDLC AI-native, o Project Manager works inside the Delivery phase com um conjunto fixo de primitivas: um agente risk-scout, quatro slash prompts, instruções escopadas, hooks validados por schema, and uma lista curada de MCPs validados. The primary outputs are status reports, a live risk register in Azure DevOps, stakeholder maps, and RAID logs that drive decisions instead of decorating meetings.

## Papel e responsabilidades

Pense no Project Manager like the air-traffic controller at a busy airport. Planes take off and land under their own power, but every takeoff and landing is sequenced, spaced, and reported to the tower. The Project Manager is that tower. Em um SDLC AI-native, o code and architecture are owned by other personas; the Project Manager owns the sequencing, the slot, and the radio discipline.
...
1. Como Project Manager, eu quero a weekly status report auto-synthesized from GitHub Projects and Azure Boards, para que I spend time on mitigation, not formatting.
2. Como Project Manager, eu quero every risk in the register with a probability, impact, owner, and review date, para que risk conversations are concrete.
3. Como Project Manager, eu quero stakeholders to know what changed and what it means for them, para que trust compounds across quarters.
4. Como Project Manager, eu quero dependencies tracked across teams with explicit acceptance criteria, para que hand-offs do not slip silently.
5. Como Project Manager, eu quero the RAID log to be the single source of truth, para que nobody re-litigates decisions in hallway conversations.
6. Como Project Manager, eu quero an audit trail of every stakeholder commitment, para que scope changes are negotiated with facts.
...
The Project Manager operates a daily and weekly loop. O loop usa primitivas do GitHub Copilot inside Visual Studio Code, Claude Code at the terminal for long-form synthesis, and Microsoft Teams via the M365 Agents SDK for stakeholder cadence.

### Setup da manhã

1. Open Visual Studio Code on the `program-ops` repository. GitHub Copilot Chat carrega the scoped `.github/instructions/pm.instructions.md`.
...
**Entrada**: A payments program with three squads, one active production incident, and an auditor review next month.
...
**Entrada**: The checkout squad depends on the identity squad delivering a new token endpoint by week 18. Evidence shows the identity squad is behind.
...
1. **Status reports that rephrase the plan**. Paraphrasing the plan is not progress. Mitigação: the `status.instructions.md` requires evidence citations for every claim.
2. **Risks without owners**. A risk without an owner is a wish. Mitigação: the pre-commit hook rejects unowned risks.
3. **Stakeholder maps that only list names**. A map without cadence and interests is a phonebook. Mitigação: `/stakeholder-map` requires cadence, interests, and preferred channel for each stakeholder.
4. **RAID logs duplicated across tools**. Multiple sources of truth create disputes. Mitigação: the RAID log lives in the `program-ops` repository; everything else is a view.
5. **Escalation by surprise**. Bad news that lands without proposed mitigations loses trust. Mitigação: the Risk Scout agent always accompanies an escalation with at least two mitigation options.
...
| L2 | Assistido | GitHub Copilot Chat for drafting, no agent, some Azure DevOps dashboards |
| L3 | Aumentado | Risk Scout agent, quatro slash prompts, instruções escopadas, RAID log in `program-ops` |
